...
class LookupTable(object):
    ...
    * 7-5-4-3-2 unsuited (worst hand possible)  => 7462
    ...
    def flushes(self):
        ...
        # 1277 + len(straight_flushes) is number of hands with all cards unique rank
    ...
    def multiples(self):
        ...
        for tp in tpgen:
            ...
            for kicker in kickers:
                ...
                product = Card.PRIMES[pair1]**2 * \
                    Card.PRIMES[pair2]**2 * Card.PRIMES[kicker]
                self.unsuited_lookup[product] = rank
                rank += 1
    ...
    def get_lexographically_next_bit_sequence(self, bits):
        ...
        if not ((t & -t) / (bits & -bits)).is_integer():
            raise Exception("Lexographic Bit Sequence needs work")
        yield next
        while True:
            t = (next | (next - 1)) + 1
            next = t | ((int((t & -t) / (next & -next)) >> 1) - 1)
            if not ((t & -t) / (next & -next)).is_integer():
                raise Exception("Lexographic Bit Sequence needs work p.2")
            yield next
